...
整理這次 LINE Cafe Bot 的功能時，我發現一件有點尷尬、但也很值得記下來的事：

**收藏功能其實以前就做過了。**

在 `line-cafe-action-agent` 版本中，我曾用 Gemini Function Calling 實作：
...
Gemini 會判斷使用者想執行哪個動作，再建立一筆等待確認的操作。使用者按下確認後，Bot 才把店家寫進 Firestore。

那一版已經證明自然語言可以控制收藏，不是假的功能，也不是只有畫面沒有資料。

所以這次更準確的說法，不是「替 Bot 第一次加入收藏」，而是：
