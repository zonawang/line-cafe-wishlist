# Rich Menu 早就做過了：我和 Codex 替「想去清單」補入口，還修掉滿版點點
...
第一版五格選單完成後，Codex 協助建立新版 Rich Menu、上傳圖片並切換 default；舊選單則先保留，避免新版本有問題時失去回復空間。

但協作沒有在 API 回覆成功後結束。我看到實際圖片上的圓點太多，直接提出「我不要這些點點」。Codex 重新檢查圖片後，先確認那不是壓縮雜訊，而是原設計中的點陣紋理；接著又發現五格版本只有 PNG，缺少可以繼續編輯的 SVG。

最後，Codex 從原本的設計結構重新整理出五格 SVG，拿掉點陣、輸出新的 2500 × 1686 PNG，檢查檔案大小與 JSON，再建立 v4 Rich Menu 並讀回目前的 default ID，確認 LINE 已經切換成功。

這段過程讓我很喜歡的一點是：我不需要先把問題翻譯成「請修改哪一行 SVG」。我只要指出真實使用感受——畫面太花、點點太多——Codex 就能往下追查素材來源、修改方式、部署步驟與驗證結果。

反過來說，Codex 也沒有替我決定什麼叫好看。是否保留點陣、想去清單放在哪裡，仍然需要我從使用者角度做判斷。這次比較像是我負責指出產品哪裡不舒服，Codex 負責把它變成可以檢查、可以部署，也可以回復的修改。
...
而 Codex 在這裡扮演的角色，也不只是產生程式碼。它把「增加入口」、「調整圖片」這些看似零碎的要求，串成從盤點舊功能、修改素材、部署到驗證的一段完整流程。
